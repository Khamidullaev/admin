...
import { getFeatureStyle } from "./style"
...
const CustomMap = ({ coordinates, protectedAreas }) => {
  const [viewport, setViewport] = useState({
    longitude: 72.4826950971688,
    latitude: 40.55741994614589,
    zoom: 15,
  })
  ...
  const coor = useMemo(() => {
    return coordinates.map((feature, index) => ({ ...feature, id: index }))[0]
  }, [coordinates])


  useEffect(() => {
      console.log("AREAS ==> ", protectedAreas)
  ...
  return (
    <div style={{ width: "100%", height: "500px" }}>
      ...
      >
        {/* <Source id="oregonjson" type="geojson" data={protectedAreas[0]}>
          <Layer
            id="anything"
            type="fill"
            source="oregonjson"
        ...
        {/* {protectedAreas.map((area, index) => (
          <Source id={`oregonjson${area.id}`} type="geojson" data={area}>
            <Layer
              id={`anything${area.id}`}
              type="fill"
              source={`oregonjson${area.id}`}
              paint={{
                "fill-color": "rgba(247, 102, 89, 1)",
                "fill-opacity": 0.6,
              }}
            />
          </Source>
        ...
        {/* <Editor features={cooor} featureStyle={getFeatureStyle} /> */}
        <Editor features={protectedAreas} featureStyle={{
          stroke: '#ff00008f',
          strokeWidth: 1,
          fill: '#ff00008f',
          fillOpacity: 0.5,
