...
const OpenMapContainer = ({ coor, setMapDrawCoordinates, mapHeight, mapWidth, gisSwitch, setGeoJSON, selectedCityId, selectedRegionId, enableDrawing }) => {
  ...
  return (
    <Map
      ...
      initialFeatures={gisSwitch ? [...coordinates , ...protectedAreas] : coordinates }
      centerCoordinates={coor?.geometry?.coordinates?.[0]?.[0]}
      mapHeight={mapHeight}
      mapWidth={mapWidth}
      enableDrawing={enableDrawing}
      onChange={val => setMapDrawCoordinates(val?.data?.[0])}
    />
  )
}
